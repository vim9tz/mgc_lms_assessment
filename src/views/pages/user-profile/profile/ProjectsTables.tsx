...
type ProjectTableProps = {
  projectTable?: ProjectTableRowType[]; // Use the correct type for projectTable
};

// Column Definitions
const columnHelper = createColumnHelper<ProjectTableRowType>()

const ProjectTables: React.FC<ProjectTableProps> = ({ projectTable = [] }) => {
  // States
  const [rowSelection, setRowSelection] = useState({});
  const [data, setData] = useState<ProjectTableRowType[]>(projectTable);
  const [globalFilter, setGlobalFilter] = useState('')

  // Ensure projectTable is not undefined and has a valid structure
  useEffect(() => {
    if (projectTable) {
      ...
      columnHelper.accessor('avatarGroup', {
        ...
        cell: ({ row }) => (
          <AvatarGroup max={4} className='flex items-center pull-up'>
            {row.original.avatarGroup?.map((avatar, index) => <CustomAvatar key={index} src={avatar} size={26} />)}
          </AvatarGroup>
        ),
        enableSorting: false
      }),
